...
- 新增 10 个基于原版统计的榜单，覆盖农业、防御、附魔、睡眠、耐久、旅行、矿业、图腾、音乐和标靶。
- 游戏内菜单按常用、战斗、建造、生存、探索分类，网页菜单增加分类与榜单搜索，避免大量榜单挤满界面。
- 游戏内新增分层点击菜单；沿用唯一的 `/leaderboard` 指令树，分类、榜单、周期和玩家参数均支持逐级 Tab 补全。
- 移除村民社交榜；历史文件中的旧 `social` 字段会被安全忽略。
- 正式 Release 提供 Minecraft 1.21.x、26.1.x、26.2.x 三个独立分发 JAR。

### 1.8.0 上游同步与历史分片

- 同步上游最新的个人榜单接收过滤、Carpet 假人识别、多线程历史扫描、`/leaderboard display on`、26.x 轮播名称颜色和网页在线状态修复。
- 每日累计快照使用 schema 5，按月压缩保存到 `world/data/rankboard/history/YYYY-MM.dat`；兼容读取并迁移 schema 3/4 与无 schema 的旧数据。
- 普通日/周/月/年榜继续使用自然日历周期；累计值低于基线时排除该玩家，不以 0 分掩盖统计回退。
- 自定义日期可以从首个可信部分快照开始，但过去日期的结束边界仍要求完整的次日零点快照。网页显示实际统计起止范围。
- 扫描期间总榜可显示带警告的缓存预览；周期榜与日期范围榜等待权威扫描完成。
- 受伤榜与近战伤害输出榜显示原版累计整数，不再除以 10。

### 功能与用途

- **多项统计**：记录食物、跳跃、挖掘、放置、综合击杀、PvP 击杀、死亡、交易、在线时间、鞘翅飞行、钓鱼、受伤害、近战伤害输出、丢弃、拾取、合成和红石元件放置，用于制作生存、活动和竞技排行榜。
- **时间周期**：提供 `daily`、`weekly`、`monthly`、`yearly` 和 `all`，方便分别查看短期活动成绩和服务器长期记录。
- **网页快捷时间**：支持最近一天、一周、一月、一季度、一年，以及保留完整的自定义起止日期查询。
- **双网页界面**：保留经典浅蓝玻璃界面，并可一键切换到深色数据中枢界面；选择会保存在浏览器本地。
- **扩展原版统计榜**：新增动物繁育榜、坚盾榜、附魔大师榜、睡神榜、工具毁灭者榜、旅行家榜、矿业大亨榜、死里逃生榜、音乐家榜和神射手榜。
- **生物受害榜**：只读取原版 `minecraft:killed_by`，提供怪物大餐、僵尸加餐、骷髅箭靶、苦力怕烟花、虫群口粮、灾厄靶子、下界燃料、末地祭品、监守者玩具等综合榜，以及 53 种单种生物受害者榜。该分类不会混入“全部”、个人统计总览或自动轮播。
- **离线数据读取**：直接读取原版 `world/stats/*.json`，即使玩家当前不在线，也能保留并查询历史成绩。
- **缓存与后台更新**：首次加载时控制读取速度，避免启动卡顿；之后只检查变化的文件，降低服务器持续开销。
- **存档数据目录**：每日累计统计快照按月压缩保存在 `world/data/rankboard/history/YYYY-MM.dat`。主状态文件由 Minecraft 管理：1.21 位于 `world/data/rankboard/rankboard_leaderboard.dat`，26.x 位于主世界维度的 `data/minecraft/rankboard/rankboard_leaderboard.dat`。
...
- 完整同步上游 1.6.0：采用正式源码实现的网页图标取色主题、图标缓存、颜色联动、菜单改进和 `world/data/rankboard/` 存档迁移。
- 保留 fork 独有的 `pvp` 玩家击杀榜和 `dealt` 近战伤害输出榜，并覆盖游戏菜单、个人/全服计分板、轮播和网页排行榜。
- 历史统计必须完成权威扫描后才参与排行；玩家退出时优先捕获内存统计，挖掘榜过滤已经失效的方块 ID。
- 日期范围支持从首个可信快照开始的部分统计，并返回实际起止日期、完整性状态以及缺失边界或统计回退警告。
- 日、周、月、年榜和个人总览统一使用日历周期；缺少可信基线或累计值发生回退的玩家不会再按 0 基线制造错误排名。
- 历史 schema 为 3。来自旧版本、可能已被反向补值污染的周期快照会废弃并重新建立；原版总榜数据不受影响。
### 1.5.8 PvP 排行榜
...
- 日期与周期快照升级为 schema 3；旧版可能被反向补值污染的周期与日期基线会自动废弃。
- 日期范围必须存在准确的开始日期和结束次日零点快照；错过零点或启动后补建的快照会拒绝查询。
- 游戏内日榜、周榜、月榜和年榜同样要求完整周期起点；中途启动建立的基线不会冒充完整周期。
...
菜单中的“查询排行榜”用于选择时间、分类和榜单后直接在聊天框显示前 10 名；它只查询，不改变侧边栏。
“分类浏览榜单”用于按分类找到榜单，再选择时间并进入操作页，可设为个人侧边栏；OP 还可设为全服侧边栏，或选择在线玩家后显示/关闭其个人侧边栏。
“查询我的统计”只汇总执行者自己的常用指标，不与其他玩家比较。
“生物受害”是独立分类，进入后再选择“综合受害榜”或“单种生物受害榜”；这些榜单不会出现在“全部”分类。

普通玩家：

```text
/leaderboard menu ranking <周期> <分类> [榜单]      选择后直接显示聊天栏排行榜
/leaderboard menu personal [周期] [分类] [榜单]      点击设置自己的侧边栏
/leaderboard menu carousel                           点击选择开启、关闭或查询轮播状态
/leaderboard menu lookmenu                           点击选择开启、关闭或查询抬头蹲起菜单状态
/leaderboard                                         打开分层点击功能菜单
/leaderboard help                                    获取指令帮助
/leaderboard menu <分类> [榜单] [周期]              浏览榜单并进入完整操作页
/leaderboard <daily|weekly|monthly|yearly|all> <分类> <榜单> [limit] 在聊天栏显示指定排行榜
/leaderboard mine <all|day|week|month|year>          查询自己的总计/本日/本周/本月/本年统计
/leaderboard display show <period> <分类> <榜单>      显示自己的客户端原版侧边栏
/leaderboard display on                              恢复关闭前的个人总览、单榜或轮播
/leaderboard display off                             关闭自己的客户端侧边栏
/leaderboard carousel <true|false|status>            开关或查询自己的榜单轮播
/leaderboard carousel color <true|false|status>       OP 设置轮播标题跟随榜单颜色或固定青色
...
```text
/leaderboard menu server [周期] [分类] [榜单]        点击设置全服共享侧边栏
/leaderboard display show <周期> <分类> <榜单> <玩家> 为指定玩家显示个人侧边栏
/leaderboard display off <玩家>                      关闭指定玩家的个人侧边栏
/leaderboard displayfilter <榜单> <true|false|status> 管理单个榜单是否允许玩家显示
/leaderboard scoreboard show <周期> <分类> <榜单>     设置全服共享的原版侧边栏
...
/leaderboard recipients <fake-only|false|whitelist|blacklist|status> 控制个人榜单数据接收者
/leaderboard cache <status|reload>                   查看或重新加载历史统计缓存
/leaderboard cache threads <0-256|status>            设置或查看并行扫描线程
...
```properties
history-files-per-second=50                 # 每个扫描线程每秒读取的文件数
history-scan-threads=0                      # 0 自动，最多使用 50% 逻辑处理器
scoreboard-recipient-filter=fake-only       # fake-only/false/whitelist/blacklist
...
website-button-enabled=true                 # 是否在菜单和帮助中显示网站按钮
...
carousel-color-follow-metric=true           # true 跟随榜单颜色；false 固定青色
...
metric-color-dealt=#FFAA00                  # 近战伤害输出榜：金色；显示原版整数
...
- Syncs upstream recipient filtering, Carpet fake-player detection, parallel history scans, `/leaderboard display on`, the 26.x carousel name-color fix, and the web online-label fix.
- Daily cumulative snapshots use schema 5 and compressed monthly files under `world/data/rankboard/history/YYYY-MM.dat`; schema 3/4 and schema-less history remain migratable.
- Daily, weekly, monthly, and yearly boards retain calendar semantics. Players whose cumulative counters moved backwards are excluded instead of being shown as zero.
- Custom ranges may start at the first trustworthy partial snapshot, but historical end boundaries still require a complete next-midnight snapshot. The web UI shows the actual evaluated range.
- All-time rankings may show a warned cache preview while scanning; period and date-range rankings wait for the authoritative scan.
- Damage taken and melee damage dealt use the original integer values and are no longer divided by ten.

### Features and purpose

- **Multiple statistics**: Tracks food, jumps, mined and placed blocks, combined kills, PvP kills, deaths, trades, playtime, elytra distance, fishing, damage taken and melee damage dealt, dropped and picked-up items, crafted items, and placed redstone components.
- **Time periods**: `daily`, `weekly`, `monthly`, `yearly`, and `all` separate short-term event results from long-term server records.
- **Mob-victim rankings**: Reads only vanilla `minecraft:killed_by` values. Aggregate and 53 per-entity victim boards live under a dedicated two-level category and are excluded from All, personal overviews, and carousel rotation.
...
- **World data directory**: Compressed daily cumulative snapshots are sharded monthly under `world/data/rankboard/history/YYYY-MM.dat`. Minecraft manages the main state file: 1.21 stores it at `world/data/rankboard/rankboard_leaderboard.dat`, while 26.x stores it in the overworld dimension data directory at `data/minecraft/rankboard/rankboard_leaderboard.dat`.
...
### 1.7.0 upstream sync and statistics improvements

- Fully syncs upstream 1.6.0, including source-based icon-derived web themes, icon caching, color consistency, menu improvements, and migration to `world/data/rankboard/`.
- Retains the fork-only `pvp` player-kill and `dealt` melee-damage-output metrics across menus, personal and global sidebars, carousel rotation, and the web dashboard.
- Rankings wait for an authoritative statistics scan; disconnects capture in-memory values first, and mined-block totals ignore stale registry IDs.
- Date ranges may start at the first trustworthy snapshot and expose actual boundaries, completeness, and warnings for missing or regressed counters.
- Daily, weekly, monthly, and yearly rankings now share calendar-period semantics. Players without a trustworthy baseline are excluded instead of being ranked from zero.
- History schema is 3. Potentially polluted legacy period snapshots are rebuilt after upgrade; vanilla all-time totals remain available.
### Installation
...
/leaderboard <period> <category> <metric> [limit]    Show a ranking in chat
/leaderboard mine <all|day|week|month|year>          Show all-time/today/this-week/this-month/this-year scores
/leaderboard display show <period> <category> <metric> Show the caller's personal sidebar
/leaderboard display on                              Restore the saved overview, single board, or carousel
...
/leaderboard carousel color <true|false|status>       Operators choose metric-following or fixed aqua carousel titles
...
```text
/leaderboard display show <period> <category> <metric> <player> Show a personal sidebar for a player
...
/leaderboard scoreboard show <period> <category> <metric> Set the server-wide vanilla sidebar
...
/leaderboard recipients <fake-only|false|whitelist|blacklist|status> Control personal-board recipients
/leaderboard cache <status|reload>                   Inspect or reload historical-stat cache
/leaderboard cache threads <0-256|status>            Set or inspect parallel scanner threads
...
```properties
history-files-per-second=50                 # Files scanned per second by each worker
history-scan-threads=0                      # 0=auto, capped at 50% of logical processors
scoreboard-recipient-filter=fake-only       # fake-only/false/whitelist/blacklist
...
website-button-enabled=true                 # Show the website button in menus and help
...
carousel-color-follow-metric=true           # Follow metric colors; false uses fixed aqua
