...
- 游戏内新增 `/lb`、`/rankboard` 快捷入口，以及 `/me`、`/show`、`/global` 直达操作；分类、榜单和周期均支持逐级 Tab 补全。
...
```text
/lb 或 /rankboard                                   直接打开可点击的快捷菜单
/leaderboard me <all|day|week|month|year>           查看自己的各周期全部统计
/leaderboard show <榜单> [周期]                     快速切换个人侧边栏（榜单优先）
/leaderboard show <周期> <榜单>                     快速切换个人侧边栏（周期优先）
/leaderboard menu <分类> [榜单]                     打开分类，或直接切换分类中的榜单
/leaderboard <core|combat|build|life|explore> <榜单>  分类直达并支持榜单补全
/leaderboard                                         换出聊天栏面板
/leaderboard help                                    获取指令帮助
/leaderboard menu <core|combat|build|life|explore|all> 分类浏览大量榜单
/leaderboard <daily|weekly|monthly|yearly|all> <metric> [limit] 选择指定的日/周/月/年/总排行榜
/leaderboard mine <all|day|week|month>               查询自己的总计/本日/本周/本月统计
...
```text
/leaderboard global <榜单> [周期]                   快速设置全局榜单（榜单优先）
/leaderboard global <周期> <榜单>                   快速设置全局榜单（周期优先）
...
/leaderboard mine <all|day|week|month>               Show all-time/today/this-week/this-month scores
