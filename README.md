...
- **多项统计**：记录食物、跳跃、挖掘、放置、综合击杀、PvP 击杀、死亡、交易、在线时间、鞘翅飞行、钓鱼、受伤害、伤害输出、丢弃、拾取、合成和红石元件放置，用于制作生存、活动和竞技排行榜。
...
- 保留 fork 独有的 `pvp` 玩家击杀榜和 `dealt` 伤害输出榜，并覆盖游戏菜单、个人/全服计分板、轮播和网页排行榜。
...
- **Multiple statistics**: Tracks food, jumps, mined and placed blocks, combined kills, PvP kills, deaths, trades, playtime, elytra distance, fishing, damage taken and dealt, dropped and picked-up items, crafted items, and placed redstone components.
...
- Retains the fork-only `pvp` player-kill and `dealt` damage-output metrics across menus, personal and global sidebars, carousel rotation, and the web dashboard.
